...
void RandomGenerators::Initialize (Kernels * kernels,
	...
	waiter_service_generator = new ExponentialGenerator(new UniformGenerator
	                                                    (kernels->GetKernel(Kernels::TWS, set_index)),
	                                                    1 / static_cast<double>(initializer_form.
		                                                    average_waiter_service_time));
	...
	cashier_service_generator = new ExponentialGenerator(new UniformGenerator
	                                                     (kernels->GetKernel(Kernels::TCS, set_index)),
	                                                     1 / static_cast<double>(initializer_form.
