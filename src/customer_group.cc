...
CustomerGroup::CustomerGroup (ChineseRestaurant * chinese_restaurant, Process * process,
                              const unsigned int current_time) : chinese_restaurant_(chinese_restaurant),
...
unsigned int CustomerGroup::PersonsInGroup ( ) const { return static_cast<unsigned int>(customer_members_.size()); }

bool CustomerGroup::IsBuffetCustomer ( ) const { return is_buffet_customer_; }

Customer* CustomerGroup::GetCustomerMember (const unsigned int position) { return customer_members_[position]; }

unsigned int CustomerGroup::GetCustomerGroupId ( ) const { return customer_group_id_; }

bool CustomerGroup::IsTerminated ( ) const { return terminated_; }

/*------------------- Actions of Process  ------------------------------*/
void CustomerGroup::AssignState (const State state) { state_ = state; }

/*------------------- Actions for Tables (for manager) ------------------------------*/
void CustomerGroup::AssignTable (Table * table) {
	if (table_ != nullptr) {
		log_->Print("Customer Group has already occupied the table #" + std::to_string(table_->GetTableId()), Log::P4,
		            Log::ERROR);
		return;
	}
	table_ = table;
	log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " is assigned to table #" + std::
	            to_string(table_->GetTableId()));
}
...
void CustomerGroup::SitOnBuffetSeats ( ) {
	...
	for (unsigned int i = 0 ; i < PersonsInGroup() ; i++) { buffet_seats_[i]->OnSit(customer_members_[i]); }
}
...
/*---------------------- Verbs for Process ------------------- */
void CustomerGroup::Activate (const unsigned int current_time) { process_->InsertEvent(new Event(this, current_time)); }
...
void CustomerGroup::Call ( ) { is_called_ = true; }

void CustomerGroup::UnCall ( ) { is_called_ = false; }

bool CustomerGroup::IsCalled ( ) const { return is_called_; }

// Action when the customer arrives to restaurant
void CustomerGroup::CustomerGroupArrives (const unsigned int current_time) {
	log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " entered the restaurant", Log::P2,
	            Log::EVENT);
	...
	if (is_buffet_customer_) {
		...
		log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " entered the buffet queue", Log::P3,
		            Log::EVENT);
	}
	else {
		chinese_restaurant_->records->PushQueueRecord({
		...
		log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " entered the restaurant queue",
		            Log::P3, Log::EVENT);
...
bool CustomerGroup::CustomerGroupWaitsInRestaurantQueue (const unsigned int current_time) {
	UnCall();
	log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " calls the manager for table", Log::P3,
	            Log::EVENT);
	// Let the manager manages if the manager is free
	if (chinese_restaurant_->manager->IsFree()) CallManager(current_time);
	return false;
}
...
bool CustomerGroup::CustomerGroupWaitsInBuffetQueue (const unsigned int current_time) {
	...
	log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " look for " + std::
	            to_string(PersonsInGroup()) + " buffet seats", Log::P3, Log::EVENT);
...
bool CustomerGroup::CustomerGroupArrivesToTable (const unsigned int current_time) {
	...
	if (!chinese_restaurant_->restaurant_queue.empty()) CallManager(current_time);
	...
	log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " enters to restaurant table", Log::P3,
	            Log::EVENT);
	return true;
}
...
bool CustomerGroup::CustomerGroupWaitsTheWaiter (const unsigned int current_time) {
	...
	if (chinese_restaurant_->free_waiter_queue.empty()) {
		log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + "  waits for a free waiter ", Log::P3,
		            Log::EVENT);
		return false;
	}
...
bool CustomerGroup::CustomerGroupInRestaurantService (const unsigned int current_time) {
	...
	log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " is served by the waiter #" + std::
	            to_string(served_by_->GetWaiterId()), Log::P3, Log::EVENT);
	return false;
}
...
bool CustomerGroup::CustomerGroupInBuffetService (const unsigned int current_time) {
	...
	Activate(current_time + service_time_);
	AssignState(kLeaveServiceState);
	log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " enters to buffet seats", Log::P3,
	            Log::EVENT);
	return false;
}
...
bool CustomerGroup::CustomerGroupLeavesService (const unsigned int current_time) {
	...
	if (!IsBuffetCustomer()) {
		...
		if (!chinese_restaurant_->wait_waiter_queue.empty() && !chinese_restaurant_->
		                                                        wait_waiter_queue.front()->IsCalled()) {
		...
		table_ = nullptr;
		log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " leaves the restaurant table", Log::P3,
		            Log::EVENT);
	}
		...
		LeaveBuffetSeats();
		log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " leaves the buffet seats", Log::P3,
		            Log::EVENT);
	}
	...
	log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " goes to the checkout queue", Log::P3,
	            Log::EVENT);
	return true;
}
...
bool CustomerGroup::CustomerGroupInCheckoutQueue (const unsigned int current_time) {
	...
	if (chinese_restaurant_->free_cashiers.empty()) {
		log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) +
		            " waits for a free cashier in the queue", Log::P3, Log::EVENT);
		return false;
	}
	...
	log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " arrives to the free cashier", Log::P3,
	            Log::EVENT);
	return true;
}
...
bool CustomerGroup::CustomerGroupInCashier (const unsigned int current_time) {
	...
	log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " starts service with the cashier #" + std::
	            to_string(cashier_->GetCashierId()), Log::P3, Log::EVENT);
	return false;
}
...
bool CustomerGroup::CustomerGroupComplete (const unsigned int current_time) {
	...
	cashier_ = nullptr;
	log_->Print("Customer Group #" + std::to_string(GetCustomerGroupId()) + " leaves the restaurant", Log::P2,
	            Log::EVENT);
	return false;
}
