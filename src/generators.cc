...
double UniformGenerator::Rand ( ) {
	const auto h = kernel_ / q_;
	kernel_     = a_ * (kernel_ - q_ * h) - r_ * h;
	if (kernel_ < 0) kernel_ = kernel_ + static_cast<int>(m_);
	return kernel_ / m_;
}

double UniformGenerator::Rand (const int start, const int end) { return Rand() * (end - start) + start; }

int UniformGenerator::GetKernel ( ) const { return kernel_; }

ExponentialGenerator::ExponentialGenerator (UniformGenerator * uniform_generator, const double lambda): lambda_(lambda),
...
NormalGenerator::NormalGenerator (UniformGenerator * uniform_generator_1,
                                  ...
                                  const double variance): uniform_generator_1_(uniform_generator_1),
...
ProbabilityGenerator (UniformGenerator * uniform_generator,
                      std::vector<double> probabilities): uniform_generator_(uniform_generator),
...
BoolProbabilityGenerator::
BoolProbabilityGenerator (UniformGenerator * uniform_generator,
                          const double true_probability): uniform_generator_(uniform_generator),
                                                          true_probability_(true_probability) { }

bool BoolProbabilityGenerator::Rand ( ) const { return uniform_generator_->Rand() <= true_probability_; }
