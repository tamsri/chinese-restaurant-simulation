...
void Manager::Manages (const unsigned int current_time) {
	...
	for (unsigned int i_customer_group = 0 ; i_customer_group < chinese_restaurant_->restaurant_queue.size() ;
	     i_customer_group++) {
		...
		for (unsigned int i_table = 0 ; i_table < chinese_restaurant_->free_restaurant_tables.size() ; i_table++) {
			...
			if (table->GetSeatNumber() >= customer_group->PersonsInGroup() && !customer_group->IsCalled()) {
				...
				chinese_restaurant_->records->PushCustomerRecord({
					                                                 customer_group->GetCustomerGroupId(), current_time,
					                                                 CustomerGroup::kRestaurantArriveTableSate
				                                                 });
				// Generate the arrival time for the group
				const auto arrive_to_table_time =
					current_time + chinese_restaurant_->variables->time_queue_to_table;
				...
				Log::GetLog()->Print("Manager assigned table #" + std::to_string(table->GetTableId())
				                     ...
				                     + " will arrive to table at " + chinese_restaurant_->
				                                                     clock->ConvertTime(arrive_to_table_time));
				...
				chinese_restaurant_->
					restaurant_queue.erase(chinese_restaurant_->restaurant_queue.begin() + i_customer_group);
				// Remove the selected table from the free tables queue
				chinese_restaurant_->free_restaurant_tables.
				                     erase(chinese_restaurant_->free_restaurant_tables.begin() + i_table);
				return;
			}
		}
	}
...
bool Manager::IsFree ( ) const { return is_free_; }

void Manager::MakeFree ( ) { is_free_ = true; }

void Manager::MakeBusy ( ) { is_free_ = false; }
