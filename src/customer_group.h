...
public:
	...
	enum State {
		// states for 
		kArriveState,
		// state when the customer group arrives to the restaurant
		// states for buffet customer group
		kBuffetQueueState,
		// state when the customer group arrives the the buffet queue
		kBuffetServiceState,
		// state when the customer group start the service after arriving to the seats
		// states for restaurant customer group
		kRestaurantQueueState,
		// state when the customer group arrives to the restaurant queue
		kRestaurantArriveTableSate,
		kRestaurantWaiterState,
		// state when the customer group arrives to the table and wait for the waiter
		kRestaurantServiceState,
		// state when the customer group start the service after a waiter arrives
		// states for checkout
		kLeaveServiceState,
		kCheckoutQueueState,
		// state when the customer group finish the service and arrive to checkout queue
		kCheckoutServiceState,
		// state when the customer group start the checkout service
	...
	void AssignState (State state);            // assign the state to the customer group for manager
...
private:
	...
	void CallManager (unsigned int current_time) const;
	// the action for customer group to call the manager to provide the table
	...
	void CustomerGroupArrives (unsigned int current_time);
	// execute arriving customer group to enter their type service
	void CreateNextCustomerGroup (unsigned int current_time) const; // execute generating the next customer group

	bool CustomerGroupWaitsTheWaiter (unsigned int current_time);
	bool CustomerGroupWaitsInRestaurantQueue (unsigned int current_time);
	// execute customer group waiting in restaurant queue
	bool CustomerGroupWaitsInBuffetQueue (unsigned int current_time); // execute customer group waiting in buffet queue
	bool CustomerGroupInCheckoutQueue (unsigned int current_time);
	// execute customer group waiting in checkout queue

	bool CustomerGroupArrivesToTable (unsigned int current_time);
	// execute customer group arriving to table by manager and waiting for the waiter
	bool CustomerGroupInRestaurantService (unsigned int current_time);
	// execute customer group starting restaurant service after the waiter arrives to the table
	bool CustomerGroupInBuffetService (unsigned int current_time);
	// execute customer group starting buffer service after arrive to the seats
	bool CustomerGroupLeavesService (unsigned int current_time);
	bool CustomerGroupInCashier (unsigned int current_time); // execute customer group 
	bool CustomerGroupComplete (unsigned int current_time);  // the customer group complete
